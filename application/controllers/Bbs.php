...
class Bbs extends CI_Controller
{
    ...
    {
        if (!empty($_SESSION['header_login'])) {
    ...
    public function edit()
    {
        if (!empty($_SESSION['admin_login'])) {
            ...
            if (!empty($id) && is_numeric($id)) {
                ...
                $_SESSION['error_message'][] = '更新に必要なメッセージが含まれていません。';
                header('location:/bbs/admin');
            }
        } else {
            header('location:/bbs/login');
    ...
    public function Auth_check()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            define('PASSWORD', '$2y$10$SbdHurka6tRt02PSRxfNMOOFUnCSSPnnFmq8RWjoTIpTTfLTKdCr6');//adminPasswordを暗号化
    ...
    public function download_csv()
    {
        ...
        if (!empty($message_array)) {
            ...
            header('location:/bbs/bbs/admin');
        }
    }

    /**
     * bbs_viewからPOSTされたデータをDBに登録する
     *
     * @return  void
     */
    public function add_bbs()
    {
        ...
        if (empty($message)) {
            $error_message[] = 'ひと言メッセージを入力して下さい。';
        }
        ...
        if (!empty($this->input->post('message_id', true))) {
            ...
            if (empty($error_message)) {
                ...
                if ($this->Bbs_model->update_record($id, $data)) {
                    ...
                    header("location: /bbs/bbs/edit?message_id={$id}");
                }
    ...
    public function delete_bbs()
    {
        if (!empty($this->input->post('message_id', true))) {
            ...
            if ($this->Bbs_model->delete_record($id)) {
                ...
                header("location: /bbs/bbs/delete?message_id={$id}");
            }
